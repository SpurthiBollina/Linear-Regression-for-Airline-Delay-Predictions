...
dm 'clear log';
%let UID= 652258189 ;
%let libpath=C:\Users\Chandana Reddy\Desktop\Spurthi\SAS\sasdata;
%let pdfpath=C:\Users\Chandana Reddy\Desktop\Spurthi\SAS ;

libname HW3 "&libpath." ;
ods pdf file="&pdfpath.\HW3-&UID..pdf" ;
%let SYSUSERI = Spurthi Bollina;

title "&SYSUSERI. - &UID. - Final Exericse" ;
...
input fil2read $155.;
datalines;
C:\Users\Chandana Reddy\Desktop\SAS HW3\On_Time_On_Time_Performance_2015_5.csv
;
*/
data HW3.BTS201505 &droprenamemv. ;
infile 'C:\Users\Chandana Reddy\Desktop\Spurthi\SAS\On_Time_On_Time_Performance_2015_5.csv'
...
proc stdize data=HW3.BTS201505_&uid. reponly missing=0 out=HW3.BTS201505_&uid. ;
...
retain DepDelayLag DepDelayLagCum 0 ArrDelayLag ArrDelayLagCum 0 CancelledLag 0 DistanceLag 0 Distance Carrier TailNum FlightDate DepDelay ArrDelay Cancelled; 
format flightdate yymmdd10. SeqNum DepDelayLag DepDelayLagInd DepDelayLagCum ArrDelayLagInd ArrDelayLag ArrDelayLagCum 4. ;
...
proc GLM data=HW3.BTS201505_&uid.  ; *descending ;
class carrier;
model DepDelay= Carrier CRSDepTime seqnum ArrDelayLagInd ArrDelayLag ArrDelayLagCum DepDelayLag2 ArrDelayLag2 CancelledLag DistanceLag;
*by carrier;
run ; 
quit ;
...
retain MaxCRSDepTime CRSDepTime Carrier TailNum FlightDate ;
format MaxCRSDepTime time5.;
set HW3.BTS201505_&uid. ; 
by Carrier TailNum FlightDate ;

if first.FlightDate=1 then do ;
	DelayCount=(DepDel15=1) ;
	lnSeqNUM = log(SeqNum) ;
	MaxCRSDepTime = CRSDepTime;
	DistanceCum=Distance ;
	CancelledCount=(Cancelled=1) ;
end ;
else do ;
	DelayCount+(DepDel15=1) ;
	lnSeqNUM = log(SeqNum) ;
...
model DelayCount= Carrier MaxCRSDepTime DistanceCum CancelledCount / type3 dist=poisson link=log offset = lnSeqNUM;
...
model DepDelay = origin CRSDepTime seqnum ArrDelayLagInd ArrDelayLag ArrDelayLagCum ArrDelayLag2 DepDelayLag2 
CancelledLag DistanceLag/ selection=LASSO(screen=sasvi ) ;
score data=HW3.BTS201505_&UID out=glmscore P R ;
...
class origin carrier ;
model DepDelay=origin CRSDepTime seqnum ArrDelayLagInd ArrDelayLag ArrDelayLagCum ArrDelayLag2 DepDelayLag2 
CancelledLag DistanceLag/ selection=LASSO(adaptive stop=none choose=SBC);
